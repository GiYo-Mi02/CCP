...
export default function LoginPage() {
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    setError("Invalid credentials. Please verify your delegate email and password and try again.");
  };
      ...
      <div className="flex-1 flex flex-col justify-center items-center p-6 sm:p-12 relative">
        <div className="w-full max-w-md bg-white rounded-3xl shadow-xl shadow-ccd-text/5 p-8 sm:p-10 border border-ccd-accent/10 relative z-10">
          ...
          <form className="space-y-6" onSubmit={handleLogin}>
            ...
            <button 
              type="submit"
              className="w-full py-4 bg-ccd-text hover:bg-ccd-active text-white rounded-xl font-bold tracking-wider uppercase text-sm transition-colors shadow-lg hover:shadow-ccd-active/20"
            >
              Sign In
            </button>
          </form>
          ...
          <button 
            type="button"
            className="w-full py-4 bg-white border-2 border-ccd-surface hover:border-ccd-accent/50 hover:bg-ccd-surface/20 text-ccd-text rounded-xl font-bold flex items-center justify-center gap-3 transition-colors group"
          >
          ...
          <div className="mt-8 text-center">
            {/* For testing only */}
            <Link href="/home" className="text-xs font-mono text-ccd-text-sec underline -mt-4 opacity-50 hover:opacity-100">
              [Bypass to Home]
            </Link>
          </div>
        </div>
