...
export default async function InsertionPeriodPage() {
    ...
    return (
      <div className="min-h-screen bg-ccd-bg flex items-center justify-center px-6">
        <div className="max-w-xl bg-white rounded-2xl border border-ccd-accent/20 p-8 text-center">
          ...
          <p className="text-ccd-text-sec mt-2">Insertion period will be available once an active session is started by the administration.</p>
        </div>
      </div>
    );
  }

  const period = sessionData.periods.find((item) => item.period_type === 'insertion');

  if (!period) {
    return (
      <div className="min-h-screen bg-ccd-bg flex items-center justify-center px-6">
        <div className="max-w-xl bg-white rounded-2xl border border-ccd-accent/20 p-8 text-center">
          <h1 className="font-serif text-3xl text-ccd-text">Insertion Period Not Configured</h1>
  ...
  return (
    <PeriodPageClient
      ...
      periodTitle="Insertion Period"
      periodDescription="Submit and vote on newly proposed constitutional sections for insertion."
